...
const TVShowUpload: React.FC = () => {
  ...
  const form = useForm<TVShowFormData>({
    ...
    defaultValues: {
      ...
      network: '',
      creator: '',
      formSeasons: [getDefaultSeasonValue()],
    }
  });
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <FormField control={form.control} name="poster" render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-white">Poster da Série *</FormLabel>
                  <FormControl>
                    <ControlledImageUpload value={field.value} onChange={field.onChange} className="h-48 w-full" />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              <FormField control={form.control} name="backdrop" render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-white">Backdrop da Série *</FormLabel>
                  <FormControl>
                    <ControlledImageUpload value={field.value} onChange={field.onChange} className="h-48 w-full" />
                  </FormControl>
                  <FormMessage />
            ...
            <div className="space-y-4 p-4 border border-gray-700 rounded-md">
              <div className="flex items-center justify-between">
                ...
                <Button 
                  ...
                  variant="outline" 
                  size="sm" 
                  onClick={() => appendSeason({...getDefaultSeasonValue(), season_number: seasonFields.length + 1 })} 
                ...
                return (
                  <div key={seasonItem.id} className="space-y-3 p-3 border border-gray-700 rounded-md bg-gray-800/20">
                    <div className="flex flex-col md:flex-row gap-4 items-start">
                      <FormField control={form.control} name={`formSeasons.${seasonIndex}.season_number`} render={({ field }) => (
                      ...
                      <FormField control={form.control} name={`formSeasons.${seasonIndex}.title`} render={({ field }) => (
                        <FormItem className="flex-1">
                          <FormLabel className="text-white">Título da Temporada</FormLabel>
                          <FormControl>
                            <Input {...field} placeholder="Ex: O Início" className="bg-gray-700 border-gray-600 text-white" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )} />
                      ...
                      >
                        ...
                        <span className="md:hidden">Remover Temporada</span>
                      </Button>
                    </div>
                    ...
                    <div className="ml-0 md:ml-4 mt-4 space-y-3 border-t border-gray-700 pt-3">
                      ...
                      {episodeFields.map((episodeItem, episodeIndex) => (
                        <div key={episodeItem.id} className="p-3 border border-gray-600 rounded-md space-y-3 bg-gray-700/40">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <FormField control={form.control} name={`formSeasons.${seasonIndex}.episodes.${episodeIndex}.episode_number`} render={({ field }) => (
                              <FormItem>
                            ...
                            <FormField control={form.control} name={`formSeasons.${seasonIndex}.episodes.${episodeIndex}.title`} render={({ field }) => (
                              <FormItem>
                          ...
                          <FormField control={form.control} name={`formSeasons.${seasonIndex}.episodes.${episodeIndex}.video_url`} render={({ field }) => (
                            <FormItem>
                              <FormLabel className="text-xs text-gray-400">Vídeo do Episódio *</FormLabel>
                              <FormControl>
                                <ControlledVideoUpload value={field.value} onChange={field.onChange} className="h-auto w-full min-h-[80px] text-xs" />
                              </FormControl>
                              <FormMessage className="text-xs" />
                            </FormItem>
                          )} />
                          
                          <FormField control={form.control} name={`formSeasons.${seasonIndex}.episodes.${episodeIndex}.synopsis`} render={({ field }) => (
                            <FormItem>
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            <FormField control={form.control} name={`formSeasons.${seasonIndex}.episodes.${episodeIndex}.poster_url`} render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-xs text-gray-400">Poster Episódio</FormLabel>
                                <FormControl>
                                  <ControlledImageUpload value={field.value} onChange={field.onChange} className="h-32 w-full text-xs" />
                                </FormControl>
                                <FormMessage className="text-xs" />
                              </FormItem>
                            )} />
                            <FormField control={form.control} name={`formSeasons.${seasonIndex}.episodes.${episodeIndex}.backdrop_url`} render={({ field }) => (
                              <FormItem>
                                <FormLabel className="text-xs text-gray-400">Backdrop Episódio</FormLabel>
                                <FormControl>
                                  <ControlledImageUpload value={field.value} onChange={field.onChange} className="h-32 w-full text-xs" />
                                </FormControl>
                                <FormMessage className="text-xs" />
                          ...
                          <FormField control={form.control} name={`formSeasons.${seasonIndex}.episodes.${episodeIndex}.duration_minutes`} render={({ field }) => (
                            <FormItem>
                      ...
                      {form.formState.errors.formSeasons?.[seasonIndex]?.episodes && !form.formState.errors.formSeasons?.[seasonIndex]?.episodes?.root && (
                        <p className="text-sm font-medium text-destructive">
                          Verifique os erros nos campos dos episódios.
                        </p>
                      )}
                    </div>
                  </div>
              ...
              {form.formState.errors.formSeasons && !form.formState.errors.formSeasons.root && (
                <p className="text-sm font-medium text-destructive">
                  Verifique os erros nos campos das temporadas.
                </p>
              )}
            </div>
